            ...
            while (size && inBytes.length >= size + 3) {
                ...
                if (isFinished) {
                    try {
                        ...
                        logger.debug({ resultLength: result.length }, 'Decrypted frame successfully');
                        frame = await (0, WABinary_1.decodeBinaryNode)(result);
                        logger.debug({ frame }, 'Decoded binary node successfully');
                    } catch (error) {
                        logger.error({ error, frameLength: frame.length }, 'Failed to decrypt or decode frame');
                        throw new Error(`Failed to decode frame: ${error.message}`);
                    }
                }
