...
$stmtSecretarios = $pdo->prepare("SELECT id_user, nome FROM usuarios WHERE funcao = 'Secretário(a)' ORDER BY nome ASC");
$stmtSecretarios->execute();
$secretarios = $stmtSecretarios->fetchAll(PDO::FETCH_ASSOC);

// Processa a finalização do atendimento
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['finalizar_atendimento'])) {
    $atendimentoId = $_POST['atendimento_id'];
    $secretarioId = $_POST['secretario_id'];

    // Atualiza o status do atendimento para "Finalizado" e salva o ID do secretário
    $stmtUpdate = $pdo->prepare('UPDATE atendimentos SET status_atendimento = ?, secretario_id = ? WHERE id = ?');
    $stmtUpdate->execute(['Finalizado', $secretarioId, $atendimentoId]);
...
$query = 'SELECT a.*, u.nome AS nome_secretario 
          FROM atendimentos a 
          LEFT JOIN usuarios u ON a.secretario_id = u.id_user 
          WHERE a.opcao_atendimento = :opcao';
$params = [':opcao' => '1'];

if ($search) {
    $query .= ' AND (a.numero LIKE :search OR a.email LIKE :search)';
    $params[':search'] = '%' . $search . '%';
}

if ($statusFilter && $statusFilter !== 'todos') {
    $query .= ' AND a.status_atendimento = :status';
    $params[':status'] = $statusFilter;
}

$query .= ' ORDER BY a.data_registro DESC LIMIT :limit OFFSET :offset';
...
<head>
    ...
    <title>Cursos Presenciais e EAD - SENAI</title>
    <link rel="stylesheet" href="../css/opcoes/cursos.css">
...
<body class="transition-colors duration-300">
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-hidden main-content lg:ml-64">
            ...
            <main class="flex-1 overflow-y-auto p-4 sm:p-6 mt-16">
                <h1 class="text-2xl sm:text-3xl font-bold text-senai-gray mb-6">Cursos Presenciais e EAD</h1>

                <!-- Seção de Pesquisa -->
                <div class="search-section">
                    <form id="searchForm" method="GET" action="cursos.php">
                ...
                <div class="filters mb-4">
                    <form id="filterForm" method="GET" action="cursos.php">
                    ...
                    <?php if (count($atendimentos) > 0): ?>
                        <?php foreach ($atendimentos as $atendimento): ?>
                            <div class="card atendimento-card shadow-custom shadow-custom-hover" data-status="<?php echo htmlspecialchars($atendimento['status_atendimento'] ?? 'Aberto'); ?>" data-search="<?php echo strtolower(htmlspecialchars(str_replace(['@s.whatsapp.net', '@s.what'], '', $atendimento['numero']) . ' ' . $atendimento['email'])); ?>">
                                ...
                                <?php if ($atendimento['nome_secretario']): ?>
                                    <p><strong>Finalizado por:</strong> <?php echo htmlspecialchars($atendimento['nome_secretario']); ?></p>
                                <?php endif; ?>
                                <div class="card-actions">
                                    ...
                                    <button onclick="window.location.href='responder.php?atendimento_id=<?php echo $atendimento['id']; ?>&numero=<?php echo urlencode(str_replace(['@s.whatsapp.net', '@s.what'], '', $atendimento['numero'])); ?>'"><i class="fas fa-comment"></i> Responder</button>                                    <?php if ($atendimento['status_atendimento'] === 'Aberto'): ?>
                                        <button onclick="openFinalizeModal(<?php echo $atendimento['id']; ?>)"><i class="fas fa-check"></i> Finalizar</button>
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p class="no-data">Nenhum atendimento registrado para Cursos Presenciais e EAD.</p>
                    <?php endif; ?>
                </div>
                ...
                <div id="finalizeModal" class="modal">
                    <div class="modal-content">
                        ...
                        <form id="finalizeForm" method="POST" action="cursos.php">
                            <label for="secretario_id">Secretário que Finalizou:</label>
                            <select id="secretario_id" name="secretario_id" required>
                                <option value="">Selecione um secretário</option>
                                <?php foreach ($secretarios as $secretario): ?>
                                    <option value="<?php echo htmlspecialchars($secretario['id_user']); ?>">
                                        <?php echo htmlspecialchars($secretario['nome']); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <input type="hidden" id="atendimento_id" name="atendimento_id">
                            <input type="hidden" name="finalizar_atendimento" value="1">
                            <div class="modal-actions">
                                <button type="submit">Confirmar</button>
                                <button type="button" onclick="closeFinalizeModal()">Cancelar</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <script src="../script/opcoes/cursos.js"></script>
</body>
</html>